...
function scrollToY(y, speed, anim, topLink) {
  ...
  if (speed) {
    ...
    if (anim && anim !== 2) {
      ...
      updSideTopLink();
      setTimeout(scrollToY.pbind(y, 100, 2), 0);
      return;
    }
    window.scroll(0, y);
    if (browser.msie6) {
...
function updGlobalPlayer() {
  ...
  var gpW = 154, gpL = vk.rtl ? pbxy[0] - gpW + 6 : pbxy[0] + pbsz[0] - 6;
  ...
  if (gpL + gpW <= dwidth && !vk.rtl || vk.rtl && gpL >= 0) {
    ...
    if (browser.mac) {
      var gpTop = (gpAbs || headFixed ? 49 + barHeight : 20);
      setStyle(_gp, {top: gpTop});
      toggleClass(_gp, 'abs', gpAbs);
    } else {
      var gpTop = headFixed ? 49 + barHeight : Math.max(Math.min(49 - scrollGetY(), 49) + barHeight, 20),
    ...
    gpL = vk.rtl ? pbxy[0] + pbsz[0] + 9 : pbxy[0] - gpW - 9;
    if (gpL + gpW <= dwidth && vk.rtl || !vk.rtl && gpL >= 0) {
      setStyle(_gb, { width: gpW + 'px' });
      setStyle(_gp, {
        width: gpW + 'px',
...
window.hideTitleProgress = function() {
  ...
  if (browser.mozilla || browser.chrome || browser.opera && !browser.opera_mobile) {
    setFavIcon('/images/favicon' + (vk.intnat ? '_vk' : 'new') + (window.devicePixelRatio >= 2 ? '_2x' : '') + '.ico');
  }
}
})();
...
function leftBlockUnpaidGiftsHide(blockId, hash) {
  return showFastBox(getLang('left_delete_unpaid_gifts_title'), getLang('left_delete_unpaid_gifts_text'), getLang('box_yes'), function() { leftBlockHide(blockId, hash); re('payments_box_unpaid_gifts_info'); curBox().hide(); }, getLang('box_no'));
}
...
function reportAd(ad_id) {
  showBox('/reports.php?act=a_report_ad_box', {ad_id: ad_id}, {params: {width: 350}, stat: ['ui_controls.js', 'ui_controls.css']});
}
...
function zNav(changed, opts, fin) {
  ...
  if (w) {
    ...
    return !showBox('/al_gifts.php', {act: 'get_gift_box', tab: 'birthday', mid: opts.id || 0, fr: (opts.is && opts.id != vk.id ? 0 : 1), link: nav.objLoc[0]}, {stat: ['gifts.css', 'ui_controls.js', 'ui_controls.css'], cache: 1}, window.event);
  }
  if (z === false || w === false) {
  ...
  if (zt) {
    ...
    switch (zt[1]) {
      ...
      case 'video_tag':
        ...
        break;
      case 'video':
        showVideo(zt[2], zt[3], extend(opts, {onFail: onFail, noLocChange: 1}));
        return false;
        break;
      case 'screen':
